...
// Kaip ir kiti tipai, masyvai gali būti naudojami funkcijų
// parametrams arba funkcijų grąžinimo tipams aprašyti
...
console.group('Panaudojimo pavyzdžiai:');
{
  ...
  {
    const couple: Array<Person> = createPeopleArray(people[0], people[2]);
    console.log(couple);
  }
  console.groupEnd();
}
...
console.group('Užduotys');
{
  ...
  {
    const numbs: number[] = [1, -8, -6, 7, 5, 1];
  ...
  console.group('2. Sukurkite ir tipais aprašykite funkciją, kuri sudarytų string\'ą iš string\'ų masyvo elementų pirmųjų raidžių');
  {
    ...
    */
    ...
    const firstLetters = (arr: string[]): string => [...arr].map((item) => item[0]).join('');
  ...
  console.group('3. Sukurkite ir tipais aprašykite funkciją, kuri sudaugintų visus number masyvo skaičius');
  {
    ...
    */
    ...
    const powNums = (arr: number[]) : number => {
      const isZero : boolean = arr.some((num) => num === 0);
      let result: number = 0;
      if (!isZero) result = arr.reduce((acc, curr) => acc * curr);
      return result;
    };
