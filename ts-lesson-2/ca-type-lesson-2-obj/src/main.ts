...
{
  type IsAdult = ({ age } : Person) => boolean;
  const isAdult:IsAdult = ({ age }) => age >= 18;
...
{
  type GetInitials = (p: Person) => string;
  const createInitials: GetInitials = (p) => `${p.name[0]}.${p.surname[0]}.`;
